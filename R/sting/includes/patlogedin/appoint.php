...
if(isset($_POST['appoint-submit'])){
    ...
    if(!mysqli_stmt_prepare($stmt,$sql)){
        ...
        $sql="INSERT INTO appointments(patid,patname,docname,appdate,apptime,status,city) VALUES(?,?,?,?,?,?,?)";
         $stmt=mysqli_stmt_init($conn);
         if(!mysqli_stmt_prepare($stmt,$sql)){
        echo sprintf("
        <form action='ntab.php' method='POST' name='myform'>
        ...
        ",$_POST['patid'],$_POST['patfname']);
            ...
            mysqli_stmt_bind_param($stmt,"sssssss",$id,$patname,$availdoc,$date,$time,$status,$city);
