    ...
    if(!mysqli_stmt_prepare($stmt,$uid)||!mysqli_stmt_prepare($stmt,$sql)){
    	...
    	exit();
    		...
    		exit();    		
    			...
    			$sql="INSERT INTO patients(patfname,patlname,patemail,patphone,password,patidlogin,city) VALUES(?,?,?,?,?,?,?)";
    			$stmt=mysqli_stmt_init($conn);
    			if(!mysqli_stmt_prepare($stmt,$sql)){
    				...
    				mysqli_stmt_bind_param($stmt,"sssssss",$fname,$lname,$email,$phone,$hashpwd,$uniqueid,$city);
    				mysqli_stmt_execute($stmt);
                    $msg='YOUR ID'.$uniqueid;
                    $path="records/".$uniqueid;
                    mkdir($path);
                    mkdir("prescriptions/".$uniqueid);
